...
class RegistryService {
  ...
  /** Pull the user's cloud installs and merge into local state. Call after sign-in. */
  async syncFromSupabase(): Promise<void> {
    ...
    if (data) {
      for (const row of data) this.installedIds.add(row.identifier);
      this.saveInstalled();
    }
  }
  ...
  install(identifier: string): void {
    ...
    this.saveInstalled();
    this.syncInstallToSupabase(identifier, true);
  }

  uninstall(identifier: string): void {
    this.installedIds.delete(identifier);
    this.saveInstalled();
    this.syncInstallToSupabase(identifier, false);
  }

  private syncInstallToSupabase(identifier: string, installed: boolean): void {
    if (!isSupabaseConfigured()) return;
    const { user } = authService.getState();
    if (!user) return;

    const mod = this.getModById(identifier);
    if (installed) {
      supabase.from('mod_installations').upsert({
        user_id: user.id,
        identifier,
        name: mod?.name ?? identifier,
        version: mod?.version ?? 'unknown',
      }).then(() => {});
    } else {
      supabase.from('mod_installations')
        .delete()
        .eq('user_id', user.id)
