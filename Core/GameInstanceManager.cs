    ...
    public class GameInstanceManager : IDisposable
    {
        ...
        public GameInstance AddInstance(GameInstance instance)
        {
            if (instance.Valid)
            {
                ...
                {
                    throw new InstanceNameTakenKraken(name);
                }
        ...
        private void LoadInstances()
        {
            ...
            foreach (Tuple<string, string, string> instance in Configuration.GetInstances())
            {
                ...
                try
                {
                    ...
                    // Use indexer instead of .Add() to handle duplicate names in config gracefully
                    instances[name] = new GameInstance(game, path, name, User);
                }
                catch (Exception exc)
                {
