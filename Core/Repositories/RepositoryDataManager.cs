...
namespace CKAN
{
    ...
    public class RepositoryDataManager
    {
        ...
        {
            var safeName = string.Join("_", (repo.name ?? "repo").Split(Path.GetInvalidFileNameChars()));
            return Directory.EnumerateFiles(reposDir)
                        .Where(path => Path.GetFileName(path).StartsWith(hash))
                        .DefaultIfEmpty(Path.Combine(reposDir, $"{hash}-{safeName}.json"))
                        .First();
        }
